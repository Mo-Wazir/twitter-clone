...
if ($getFromUser->loggedIn() === false) {
	...
	if (isset($_POST['tweet'])) {
		...
		if(!empty($status) or !empty($_FILES['file']['name'][0])){
			...
			$getFromUser->create('tweets', array('status' => $status, 'tweetBy' => $user_id, 'tweetImage' => $tweetImage, 'postedOn' => date('Y-m-d H:i:s')));
		} else {
			$error = "type or choose to tweet";
		}
 	}
			...
			<div class="info-inner">
				...
				<div class="info-in-body">
					...
					<div class="info-body-name">
						<div class="in-b-name">
							<div> <a href="<?= $user->username; ?>">
							<?= $user->screenName; ?>
							</a> </div>
							<span><small><a href="<?= $user->profileCover; ?>"><?= $user->username; ?></a></small></span>
						</div><!-- in b name end-->
						 ...
						 <div class="tweet-footer">
						 	<div class="t-fo-left">
						 		<ul>
						 			...
						 			<span class="tweet-error"><?  if(isset($error)){echo $error;} elseif(isset($imageError)) {echo $imageError;} ?></span>
						 			</li>
						 		</ul>
						 	</div>
